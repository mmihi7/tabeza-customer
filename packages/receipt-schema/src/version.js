...
exports.VERSION_REGISTRY = exports.SemanticVersionSchema = exports.MINIMUM_SUPPORTED_VERSION = exports.CURRENT_SCHEMA_VERSION = void 0;
exports.parseVersion = parseVersion;
exports.compareVersions = compareVersions;
exports.checkVersionCompatibility = checkVersionCompatibility;
exports.getMigrationPath = getMigrationPath;
exports.isDataCompatible = isDataCompatible;
exports.migrateData = migrateData;
exports.validateAgentCompatibility = validateAgentCompatibility;
...
exports.MINIMUM_SUPPORTED_VERSION = '1.0.0';
/**
 * Semantic version schema
 */
exports.SemanticVersionSchema = zod_1.z.string().regex(/^\d+\.\d+\.\d+(?:-[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*)?(?:\+[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*)?$/, 'Invalid semantic version format');
// ============================================================================
// VERSION REGISTRY
// ============================================================================
/**
...
function parseVersion(version) {
    const result = exports.SemanticVersionSchema.safeParse(version);
    if (!result.success) {
 ...
 */
function checkVersionCompatibility(requestedVersion) {
    const warnings = [];
    try {
        // Validate version format
        exports.SemanticVersionSchema.parse(requestedVersion);
    }
    catch (error) {
        return {
            compatible: false,
            currentVersion: exports.CURRENT_SCHEMA_VERSION,
            requestedVersion,
    ...
    if (!exports.VERSION_REGISTRY[requestedVersion]) {
        return {
            compatible: false,
            currentVersion: exports.CURRENT_SCHEMA_VERSION,
            requestedVersion,
    ...
    const comparison = compareVersions(requestedVersion, exports.CURRENT_SCHEMA_VERSION);
    const minComparison = compareVersions(requestedVersion, exports.MINIMUM_SUPPORTED_VERSION);
    ...
    if (minComparison < 0) {
        return {
            ...
            currentVersion: exports.CURRENT_SCHEMA_VERSION,
            requestedVersion,
            reason: `Version ${requestedVersion} is no longer supported. Minimum supported version is ${exports.MINIMUM_SUPPORTED_VERSION}`,
            migrationRequired: true,
            migrationPath: getMigrationPath(requestedVersion, exports.CURRENT_SCHEMA_VERSION),
            warnings
        };
    }
    ...
    if (comparison > 0) {
        return {
            ...
            currentVersion: exports.CURRENT_SCHEMA_VERSION,
            requestedVersion,
            reason: `Version ${requestedVersion} is newer than current version ${exports.CURRENT_SCHEMA_VERSION}. Please upgrade the schema package.`,
            migrationRequired: false,
            warnings
        };
    }
    ...
    const migrationPath = migrationRequired ? getMigrationPath(requestedVersion, exports.CURRENT_SCHEMA_VERSION) : undefined;
    // Add warnings for deprecated features
    const versionMetadata = exports.VERSION_REGISTRY[requestedVersion];
 ...
 */
function isDataCompatible(data, targetVersion) {
 ...
 */
function validateAgentCompatibility(agentVersion) {
    const compatibility = checkVersionCompatibility(agentVersion);
    if (!compatibility.compatible) {
        if (compatibility.reason?.includes('newer than current')) {
            return {
                compatible: false,
                message: `Agent version ${agentVersion} is newer than schema version ${exports.CURRENT_SCHEMA_VERSION}. Please upgrade the schema package.`,
            ...
            return {
                compatible: false,
                message: `Agent version ${agentVersion} is no longer supported. Please upgrade to version ${exports.MINIMUM_SUPPORTED_VERSION} or later.`,
                action: 'upgrade_required'
            };
        }
        else {
 ...
 */
function hasBreakingChanges(fromVersion, toVersion) {
    const from = parseVersion(fromVersion);
    const to = parseVersion(toVersion);
    ...
    const versions = Object.keys(exports.VERSION_REGISTRY)
        .filter(v => compareVersions(v, fromVersion) > 0 && compareVersions(v, toVersion) <= 0)
        .sort(compareVersions);
    return versions.some(version => exports.VERSION_REGISTRY[version].breakingChanges.length > 0);
}
/**
...
function getBreakingChanges(fromVersion, toVersion) {
    ...
    versions.forEach(version => {
        breakingChanges.push(...exports.VERSION_REGISTRY[version].breakingChanges);
    });
 ...
 */
function getMigrationInstructions(fromVersion, toVersion) {
    const versions = Object.keys(exports.VERSION_REGISTRY)
        .filter(v => compareVersions(v, fromVersion) > 0 && compareVersions(v, toVersion) <= 0)
        .sort(compareVersions);
    const instructions = [];
    versions.forEach(version => {
        const metadata = exports.VERSION_REGISTRY[version];
