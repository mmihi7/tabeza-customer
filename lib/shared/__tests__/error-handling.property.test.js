...
import { sanitizeErrorMessage, categorizeError, createErrorLogEntry, formatErrorForUser, isRetryableError, } from '../error-handling';
describe('Feature: fix-close-tab-errors, Property 8: Error Message Safety', () => {
    ...
    it('should remove database table names from error messages', () => {
        fc.assert(fc.property(fc.constantFrom('tabs', 'tab_orders', 'tab_payments', 'bars', 'users', 'audit_logs'), (tableName) => {
    ...
    it('should remove PostgreSQL error codes from error messages', () => {
        fc.assert(fc.property(fc.constantFrom('23505', '23503', '42501', '42P01', 'P0001'), (errorCode) => {
    ...
    it('should always return non-empty sanitized messages', () => {
        fc.assert(fc.property(fc.oneof(fc.string({ minLength: 0, maxLength: 100 }), fc.constant(null), fc.constant(undefined), fc.constant({})), (errorInput) => {
...
describe('Feature: fix-close-tab-errors, Property 9: Error Logging Completeness', () => {
    ...
    it('should consistently categorize errors by type', () => {
        fc.assert(fc.property(fc.constantFrom({ code: 'PGRST301', expectedType: 'network' }, { status: 401, expectedType: 'permission' }, { status: 400, expectedType: 'validation' }, { code: 'P0001', expectedType: 'database' }), ({ code, status, expectedType }) => {
    ...
    it('should correctly identify retryable errors', () => {
        fc.assert(fc.property(fc.constantFrom({ code: 'PGRST301', shouldRetry: true }, { status: 401, shouldRetry: false }, { status: 400, shouldRetry: false }, { code: 'P0001', shouldRetry: true }), ({ code, status, shouldRetry }) => {
            // Create error
            const error = new Error('Test error');
            if (code)
                error.code = code;
    ...
    it('should preserve original error in log entries', () => {
        fc.assert(fc.property(fc.string({ minLength: 10, maxLength: 100 }), (originalMessage) => {
            // Create error
