...
async function simulateVenueConfiguration(page, config) {
    ...
    await page.addInitScript((configData) => {
        ...
        window.__VISUAL_TEST_CONFIG__ = configData;
    }, config);
...
async function waitForThemeApplication(page, expectedTheme) {
    ...
    for (const config of VENUE_CONFIGURATIONS) {
        test.describe(`${config.name}`, () => {
            test('should display correct theme colors and icons in settings page', async ({ page }) => {
                ...
                await expect(page).toHaveScreenshot(`settings-${config.venueMode}-${config.authorityMode}-theme.png`, {
                    fullPage: true,
                    animations: 'disabled'
                });
                ...
                if (await themeIndicator.count() > 0) {
                    await expect(themeIndicator).toHaveScreenshot(`theme-indicator-${config.venueMode}-${config.authorityMode}.png`);
                }
            });
            test('should display correct theme in venue configuration display', async ({ page }) => {
                ...
                if (await configDisplay.count() > 0) {
                    await expect(configDisplay).toHaveScreenshot(`config-display-${config.venueMode}-${config.authorityMode}.png`);
                }
            });
            test('should maintain theme consistency in onboarding modal', async ({ page }) => {
                ...
                await expect(modal).toHaveScreenshot(`onboarding-modal-initial.png`);
                ...
                await expect(modal).toHaveScreenshot(`onboarding-modal-${config.venueMode}-selected.png`);
                ...
                if (config.venueMode === 'venue') {
                    ...
                    await expect(modal).toHaveScreenshot(`onboarding-modal-${config.venueMode}-${config.authorityMode}-authority.png`);
                    // Continue to summary
                    await page.click('button:has-text("Continue")');
                    await page.waitForTimeout(500);
                }
                // Take screenshot of summary step with theme applied
                await expect(modal).toHaveScreenshot(`onboarding-modal-${config.venueMode}-${config.authorityMode}-summary.png`);
            });
        });
    }
});
        ...
        test.describe(`${screenSize.name} (${screenSize.width}x${screenSize.height})`, () => {
            ...
            for (const config of VENUE_CONFIGURATIONS) {
                test(`should display ${config.name} correctly on ${screenSize.name}`, async ({ page }) => {
                    ...
                    await expect(page).toHaveScreenshot(`responsive-${screenSize.name.toLowerCase().replace(' ', '-')}-${config.venueMode}-${config.authorityMode}.png`, {
                        fullPage: true,
                        animations: 'disabled'
                    });
                });
            ...
            test(`should display onboarding modal correctly on ${screenSize.name}`, async ({ page }) => {
                ...
                await expect(page).toHaveScreenshot(`onboarding-responsive-${screenSize.name.toLowerCase().replace(' ', '-')}.png`, {
                    fullPage: true,
                    animations: 'disabled'
                });
            });
    ...
    for (const config of VENUE_CONFIGURATIONS) {
        test(`should display correct icons for ${config.name}`, async ({ page }) => {
            ...
            if (await iconContainer.count() > 0) {
                await expect(iconContainer).toHaveScreenshot(`icons-${config.venueMode}-${config.authorityMode}.png`);
            }
        });
        test(`should use correct color scheme for ${config.name}`, async ({ page }) => {
            ...
            if (await colorContainer.count() > 0) {
                await expect(colorContainer).toHaveScreenshot(`colors-${config.venueMode}-${config.authorityMode}.png`);
            }
        });
    }
});
