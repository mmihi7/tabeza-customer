...
async function triggerOnboardingModal(page, forced = false) {
...
async function simulateValidationErrors(page) {
    await page.addInitScript(() => {
        // Mock validation errors
        window.__MOCK_VALIDATION_ERRORS__ = [
...
async function simulateNetworkIssues(page) {
    ...
    test.describe('Modal States', () => {
        for (const state of MODAL_STATES) {
            test(`should display correctly in ${state.name} state`, async ({ page }) => {
                ...
                await expect(modal).toHaveScreenshot(`onboarding-modal-${state.name.toLowerCase().replace(/\s+/g, '-')}.png`);
                // Test modal backdrop
                const backdrop = page.locator('[data-testid="modal-backdrop"]');
                if (await backdrop.count() > 0) {
                    await expect(page).toHaveScreenshot(`onboarding-backdrop-${state.name.toLowerCase().replace(/\s+/g, '-')}.png`, { fullPage: true });
                }
            });
        }
    });
    ...
    test.describe('Responsive Modal Design', () => {
        ...
        for (const breakpoint of responsiveBreakpoints) {
            test(`should display correctly on ${breakpoint.name}`, async ({ page }) => {
                ...
                await expect(modal).toHaveScreenshot(`onboarding-modal-${breakpoint.name.toLowerCase()}.png`);
                // Test modal positioning and sizing
                await expect(page).toHaveScreenshot(`onboarding-modal-${breakpoint.name.toLowerCase()}-fullscreen.png`, { fullPage: true });
                // Test each step at this breakpoint
                for (const step of ONBOARDING_STEPS) {
                    if (step.step === 'mode') {
                        // Already on mode step
                        await expect(modal).toHaveScreenshot(`onboarding-${step.step}-${breakpoint.name.toLowerCase()}.png`);
                    }
