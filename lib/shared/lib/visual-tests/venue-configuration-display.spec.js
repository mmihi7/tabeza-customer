...
async function setupConfigurationDisplay(page, config) {
    ...
    test.describe('Configuration Display by Mode', () => {
        for (const testCase of CONFIGURATION_TEST_CASES) {
            test(`should display ${testCase.name} correctly`, async ({ page }) => {
                ...
                await expect(configDisplay).toHaveScreenshot(`config-display-${testCase.config.venue_mode}-${testCase.config.authority_mode}.png`);
                // Test header section with theme
                const header = configDisplay.locator('[data-testid="config-header"]');
                if (await header.count() > 0) {
                    await expect(header).toHaveScreenshot(`config-header-${testCase.config.venue_mode}-${testCase.config.authority_mode}.png`);
                }
                ...
                if (await enabledFeatures.count() > 0) {
                    await expect(enabledFeatures).toHaveScreenshot(`enabled-features-${testCase.config.venue_mode}-${testCase.config.authority_mode}.png`);
                }
                ...
                if (await disabledFeatures.count() > 0) {
                    await expect(disabledFeatures).toHaveScreenshot(`disabled-features-${testCase.config.venue_mode}-${testCase.config.authority_mode}.png`);
                }
                ...
                if (await limitations.count() > 0) {
                    await expect(limitations).toHaveScreenshot(`limitations-${testCase.config.venue_mode}-${testCase.config.authority_mode}.png`);
                }
                // Test configuration summary section
                const summary = configDisplay.locator('[data-testid="configuration-summary"]');
                if (await summary.count() > 0) {
                    await expect(summary).toHaveScreenshot(`summary-${testCase.config.venue_mode}-${testCase.config.authority_mode}.png`);
                }
                ...
                if (await themeIndicator.count() > 0) {
                    await expect(themeIndicator).toHaveScreenshot(`theme-indicator-${testCase.config.venue_mode}-${testCase.config.authority_mode}.png`);
                }
            });
        }
    });
            ...
            for (const testCase of CONFIGURATION_TEST_CASES) {
                ...
                for (const feature of testCase.expectedFeatures.enabled) {
                    ...
                    if (await featureElement.count() > 0) {
                        ...
                        await expect(featureContainer).toHaveScreenshot(`feature-enabled-${feature.toLowerCase().replace(/\s+/g, '-')}-${testCase.config.venue_mode}-${testCase.config.authority_mode}.png`);
                    }
                }
            }
        });
            ...
            for (const testCase of CONFIGURATION_TEST_CASES) {
                ...
                for (const feature of testCase.expectedFeatures.disabled) {
                    ...
                    if (await featureElement.count() > 0) {
                        // Get the parent container to capture the full feature display
                        const featureContainer = featureElement.locator('..').locator('..');
                        await expect(featureContainer).toHaveScreenshot(`feature-disabled-${feature.toLowerCase().replace(/\s+/g, '-')}-${testCase.config.venue_mode}-${testCase.config.authority_mode}.png`);
                    }
                }
            }
        });
        ...
        for (const breakpoint of breakpoints) {
            test(`should display correctly on ${breakpoint.name}`, async ({ page }) => {
                ...
                for (const testCase of CONFIGURATION_TEST_CASES) {
                    ...
                    await expect(configDisplay).toHaveScreenshot(`config-display-${testCase.config.venue_mode}-${testCase.config.authority_mode}-${breakpoint.name.toLowerCase()}.png`);
                    // Test grid layout responsiveness
                    const featuresGrid = configDisplay.locator('[data-testid="features-grid"]');
                    if (await featuresGrid.count() > 0) {
                        await expect(featuresGrid).toHaveScreenshot(`features-grid-${testCase.config.venue_mode}-${testCase.config.authority_mode}-${breakpoint.name.toLowerCase()}.png`);
                    }
                }
            });
        }
    ...
    test.describe('Interactive Elements', () => {
        test('should display tooltips correctly', async ({ page }) => {
            ...
            for (let i = 0; i < Math.min(triggerCount, 3); i++) {
                ...
                await expect(configDisplay).toHaveScreenshot(`tooltip-${i}-visible.png`);
        ...
        test('should display expandable sections correctly', async ({ page }) => {
            ...
            for (let i = 0; i < sectionCount; i++) {
                ...
                await expect(section).toHaveScreenshot(`expandable-section-${i}-collapsed.png`);
                // Click to expand
                ...
                if (await expandButton.count() > 0) {
                    ...
                    await expect(section).toHaveScreenshot(`expandable-section-${i}-expanded.png`);
                }
            }
        });
    });
        ...
        test('should apply theme colors consistently', async ({ page }) => {
            for (const testCase of CONFIGURATION_TEST_CASES) {
                ...
                for (let i = 0; i < Math.min(elementCount, 5); i++) {
                    ...
                    await expect(element).toHaveScreenshot(`theme-element-${testCase.expectedTheme}-${i}.png`);
                }
                ...
                for (let i = 0; i < Math.min(textCount, 3); i++) {
                    ...
                    await expect(element).toHaveScreenshot(`theme-text-${testCase.expectedTheme}-${i}.png`);
                }
            }
        });
        test('should display theme icons correctly', async ({ page }) => {
            for (const testCase of CONFIGURATION_TEST_CASES) {
                ...
                if (await iconContainer.count() > 0) {
                    await expect(iconContainer).toHaveScreenshot(`theme-icons-${testCase.config.venue_mode}-${testCase.config.authority_mode}.png`);
                }
                ...
                for (let i = 0; i < Math.min(iconCount, 5); i++) {
                    ...
                    await expect(icon).toHaveScreenshot(`feature-icon-${i}-${testCase.config.venue_mode}-${testCase.config.authority_mode}.png`);
                }
            }
        });
    });
