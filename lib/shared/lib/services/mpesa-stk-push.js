 ...
 */
export async function sendSTKPush(request, config) {
    ...
    if (lastError instanceof TypeError && lastError.message.includes('Failed to fetch')) {
        throw new STKPushError(`Network error: STK Push failed after ${maxRetries} attempts`, undefined, undefined, undefined, lastError);
    }
    throw new STKPushError(`STK Push failed after ${maxRetries} attempts: ${lastError?.message || 'Unknown error'}`, undefined, undefined, undefined, lastError);
}
/**
 * Attempt a single STK Push request
 */
async function attemptSTKPush(request, normalizedPhone, config) {
    try {
        // Get OAuth token
        ...
        if (!response.ok) {
            ...
            const description = errorData?.ResponseDescription
                ?? errorData?.errorMessage
                ?? (errorText && errorText.length < 300 ? errorText : null);
            const detail = description ? ` - ${description}` : (errorText ? ` - ${errorText.slice(0, 200)}` : '');
            throw new STKPushError(`STK Push request failed: ${response.status} ${response.statusText}${detail}`, errorData?.ResponseCode, errorData?.ResponseDescription || errorText, response.status, errorData);
        }
        const responseData = await response.json();
        // Validate response format
        if (!responseData.ResponseCode || !responseData.CheckoutRequestID) {
            throw new STKPushError('Invalid STK Push response format from Safaricom API', responseData.ResponseCode, responseData.ResponseDescription, undefined, responseData);
        }
        // Check if the response indicates an error
        if (responseData.ResponseCode !== '0') {
            throw new STKPushError(`STK Push rejected by Safaricom: ${responseData.ResponseDescription}`, responseData.ResponseCode, responseData.ResponseDescription, undefined, responseData);
        }
        return responseData;
    }
    catch (error) {
        ...
        if (error instanceof MpesaOAuthError) {
            throw new STKPushError(`Authentication failed: ${error.message}`, undefined, undefined, error.statusCode, error);
        }
        ...
        // Wrap other errors
        throw new STKPushError(`STK Push request failed: ${error instanceof Error ? error.message : 'Unknown error'}`, undefined, undefined, undefined, error);
    }
}
/**
 * Validate STK Push request parameters
 */
function validateSTKPushRequest(request) {
    const errors = [];
 ...
 */
export function getSTKPushStats() {
    return {
 ...
 */
export function resetSTKPushStats() {
 ...
 */
export async function sendSTKPushWithStats(request, config) {
