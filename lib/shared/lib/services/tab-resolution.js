...
export class TabNotFoundError extends Error {
    constructor(message, strategy, originalError) {
        super(message);
...
export class TabResolver {
     ...
     */
    async resolveTab(tabId, deviceId) {
     ...
     */
    async deviceIdCrossReference(deviceId) {
        if (!deviceId) {
     ...
     */
    async sessionDataRepair(tabId, deviceId) {
     ...
     */
    async recentTabFallback(deviceId) {
 ...
 */
export async function resolveTabForPayment(tabId) {
    ...
    if (!result.success || !result.tab) {
        throw new TabNotFoundError(result.error || 'Tab resolution failed', result.strategy);
    }
    return result.tab;
}
/**
 ...
 */
export function getTabNotFoundErrorMessage(strategy, error) {
