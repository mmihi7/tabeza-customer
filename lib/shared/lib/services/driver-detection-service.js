 ...
 */
import { DriverDetectionError, UnsupportedPlatformError, } from './printer-service-types';
/**
 ...
 */
export function generateInstallationGuidance(platform) {
 ...
 */
export async function performDriverDetection(driversRequired) {
    try {
        // Detect platform
        ...
        const platform = detectPlatform();
        throw new DriverDetectionError(platform, error instanceof Error ? error.message : 'Unknown error during driver detection');
    }
}
/**
 ...
 */
export function getPlatformDescription(platform) {
    const osNames = {
        windows: 'Windows',
        macos: 'macOS',
        linux: 'Linux',
