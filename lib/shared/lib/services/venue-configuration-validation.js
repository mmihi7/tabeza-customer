 ...
 */
export function validateVenueConfiguration(input) {
    const errors = [];
    const warnings = [];
 ...
 */
export function generateCorrectedConfiguration(input) {
 ...
 */
export function validateConfigurationChange(currentConfig, newConfig) {
    const errors = [];
    const warnings = [];
    ...
    if (currentConfig.venue_mode === 'venue' && newConfig.venue_mode === 'basic') {
        warnings.push('Switching from Venue to Basic mode will disable customer ordering and menus. ' +
    ...
    if (currentConfig.authority_mode === 'tabeza' && newConfig.authority_mode === 'pos') {
        warnings.push('Switching from Tabeza to POS authority will require POS integration setup. ' +
    ...
    if (currentConfig.authority_mode === 'pos' && newConfig.authority_mode === 'tabeza') {
        warnings.push('Switching from POS to Tabeza authority will disable POS integration. ' +
 ...
 */
export function isValidCoreConfiguration(config) {
 ...
 */
export function getConfigurationDescription(config) {
    if (config.venue_mode === 'basic') {
 ...
 */
export function getThemeConfiguration(config) {
    if (config.venue_mode === 'basic') {
        return {
            theme: 'blue',
 ...
 */
export function getDefaultMigrationConfiguration() {
