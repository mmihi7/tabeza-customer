 ...
 */
async function requestNewToken(config, cacheKey) {
    try {
        ...
        if (responseText.includes('<!DOCTYPE') || responseText.includes('<html')) {
            ...
            throw new MpesaOAuthError(`HTTP ${response.status}: ${errorMessage}`, response.status, responseText);
        }
        if (!response.ok) {
            throw new MpesaOAuthError(`OAuth failed ${response.status}: ${responseText}`, response.status, responseText);
        }
        let tokenResponse;
        try {
            ...
            throw new MpesaOAuthError(`OAuth returned non-JSON: ${responseText}`, response.status, responseText);
        }
        if (!tokenResponse.access_token) {
            throw new MpesaOAuthError(`OAuth missing access_token: ${responseText}`, response.status, responseText);
        }
        ...
        if (error instanceof TypeError && error.message.includes('Failed to fetch')) {
            throw new MpesaOAuthError('Network error while requesting OAuth token', undefined, error);
        }
        // Handle AbortError (timeout)
        if (error instanceof Error && error.name === 'AbortError') {
            throw new MpesaOAuthError('OAuth request timed out after 10 seconds', undefined, error);
        }
        ...
        // Wrap other errors
        throw new MpesaOAuthError(`OAuth token request failed: ${error instanceof Error ? error.message : 'Unknown error'}`, undefined, error);
    }
}
/**
