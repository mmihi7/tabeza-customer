 ...
 */
export async function getActiveDrivers(barId) {
 ...
 */
export async function getAllDrivers(barId) {
    const supabase = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL, process.env.SUPABASE_SECRET_KEY);
    const { data, error } = await supabase
        .from('printer_drivers')
        .select('*')
 ...
 */
export function isDriverActive(lastHeartbeat) {
 ...
 */
export function getTimeSinceHeartbeat(lastHeartbeat) {
