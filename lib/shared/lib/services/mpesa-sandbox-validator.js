 ...
 */
import { getSandboxTestPhoneNumbers } from './mpesa-config';
/**
 ...
 */
export function validateSandboxConfiguration(config, options = {}) {
    const result = {
        isValid: true,
        errors: [],
        warnings: [],
 ...
 */
function validateBusinessShortcode(config, result) {
    ...
    if (config.businessShortcode !== STANDARD_SANDBOX_SHORTCODE) {
        result.warnings.push(`Business shortcode "${config.businessShortcode}" is not the standard sandbox shortcode "${STANDARD_SANDBOX_SHORTCODE}". ` +
 ...
 */
function validateConsumerCredentials(config, result) {
 ...
 */
function validatePasskey(config, result) {
    ...
    if (config.passkey !== STANDARD_SANDBOX_PASSKEY) {
        result.warnings.push('Passkey is not the standard sandbox passkey. This may work if you have a custom sandbox app, ' +
 ...
 */
function validateCallbackUrl(config, result, checkAccessibility) {
    ...
    if (devDomains.some(domain => url.hostname.includes(domain))) {
        result.warnings.push(`Callback URL uses development domain (${url.hostname}). ` +
    ...
    if (checkAccessibility) {
        result.recommendations.push('Consider testing callback URL accessibility by making a test POST request to ensure Safaricom can reach it.');
    }
}
/**
 * Validate API URLs for sandbox
 */
function validateApiUrls(config, result) {
 ...
 */
function addSandboxRecommendations(result) {
    const testPhoneNumbers = getSandboxTestPhoneNumbers();
    result.recommendations.push('Use Safaricom test phone numbers for testing: ' + testPhoneNumbers.join(', '));
    result.recommendations.push('Test with small amounts (1-1000 KES) to avoid unnecessary charges in sandbox');
    result.recommendations.push('Monitor callback logs during testing as sandbox callbacks may be delayed');
    result.recommendations.push('Ensure your Safaricom Developer Portal app has M-Pesa permissions enabled');
    result.recommendations.push('Test OAuth token generation before attempting payments to validate credentials');
}
/**
 ...
 */
export function validateSandboxPhoneNumber(phoneNumber) {
    if (!phoneNumber) {
        return {
            isValid: false,
 ...
 */
export function getSandboxConfigurationSummary(config) {
