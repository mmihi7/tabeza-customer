...
export class ConfigurationHistoryService {
     ...
     */
    async getConfigurationHistory(options) {
        try {
            const { barId, limit = 50, offset = 0, eventTypes, startDate, endDate, userId } = options;
            // Build query
            ...
            const formattedEntries = (data || []).map(entry => this.formatHistoryEntry(entry));
            return {
     ...
     */
    async getRecentConfigurationChanges(barId) {
     ...
     */
    async getConfigurationTimestamps(barId) {
        try {
     ...
     */
    getConfigurationEventTypes(eventTypes) {
     ...
     */
    getActionDisplayInfo(action, details) {
        switch (action) {
 ...
 */
let configHistoryServiceInstance = null;
export function getConfigurationHistoryService() {
 ...
 */
export async function getVenueConfigurationHistory(barId, options) {
...
export async function getRecentConfigurationChanges(barId) {
    const service = getConfigurationHistoryService();
    return service.getRecentConfigurationChanges(barId);
}
export async function getConfigurationTimestamps(barId) {
    const service = getConfigurationHistoryService();
