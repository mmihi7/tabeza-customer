 ...
 */
export function loadMpesaConfigFromBar(barData) {
    ...
    if (!barData.mpesa_environment)
        missingFields.push('mpesa_environment');
    ...
    if (!barData.mpesa_business_shortcode)
        missingFields.push('mpesa_business_shortcode');
    if (!barData.mpesa_consumer_key_encrypted)
        missingFields.push('mpesa_consumer_key_encrypted');
    if (!barData.mpesa_consumer_secret_encrypted)
        missingFields.push('mpesa_consumer_secret_encrypted');
    if (!barData.mpesa_passkey_encrypted)
        missingFields.push('mpesa_passkey_encrypted');
    // Requirement 4.4: Return clear error messages for missing configuration
    if (missingFields.length > 0) {
        throw new MpesaConfigurationError(`Missing required M-Pesa configuration: ${missingFields.join(', ')}`, missingFields);
    }
    ...
    try {
        ...
        throw new MpesaConfigurationError(`Failed to decrypt M-Pesa credentials: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
 ...
 */
function loadSandboxConfig(barData, missingFields, callbackUrl) {
    // For sandbox, we need individual consumer key and secret, but can use standard business shortcode and passkey
    const sandboxMissingFields = [];
    if (!barData.mpesa_consumer_key_encrypted)
        sandboxMissingFields.push('mpesa_consumer_key_encrypted');
    if (!barData.mpesa_consumer_secret_encrypted)
        sandboxMissingFields.push('mpesa_consumer_secret_encrypted');
    ...
    if (sandboxMissingFields.length > 0) {
        throw new MpesaConfigurationError(`Missing required M-Pesa sandbox configuration: ${sandboxMissingFields.join(', ')}. ` +
            `For sandbox testing, you need your own Consumer Key and Consumer Secret from https://developer.safaricom.co.ke/MyApps`, sandboxMissingFields);
    }
    ...
    try {
        ...
        throw new MpesaConfigurationError(`Failed to decrypt M-Pesa sandbox credentials: ${error instanceof Error ? error.message : 'Unknown error'}. ` +
...
function validateEnvironment(env) {
    if (!env) {
        throw new MpesaConfigurationError('Missing mpesa_environment in bar configuration');
    }
    const normalizedEnv = env.toLowerCase();
    if (normalizedEnv !== 'sandbox' && normalizedEnv !== 'production') {
        throw new MpesaConfigurationError(`Invalid mpesa_environment: "${env}". Must be "sandbox" or "production"`);
    }
...
function validateConfig(config) {
    ...
    if (errors.length > 0) {
        throw new MpesaConfigurationError(`Invalid M-Pesa configuration: ${errors.join(', ')}`);
    }
}
/**
 ...
 */
export function getSandboxTestPhoneNumbers() {
    return [
 ...
 */
export function isSandboxTestPhoneNumber(phoneNumber) {
 ...
 */
export function isMpesaConfiguredForBar(barData) {
    try {
 ...
 */
export function loadMpesaConfig() {
    ...
    if (!businessShortcode)
        missingVariables.push('MPESA_BUSINESS_SHORTCODE');
    if (!passkey)
        missingVariables.push('MPESA_PASSKEY');
    if (!callbackUrl)
        missingVariables.push('MPESA_CALLBACK_URL');
    if (missingVariables.length > 0) {
        throw new MpesaConfigurationError(`Missing required M-Pesa environment variables: ${missingVariables.join(', ')}`, missingVariables);
    }
    const urls = getEnvironmentUrls(environment);
    const config = {
        environment,
 ...
 */
function getEnvironmentFromEnvVars() {
    const env = process.env.MPESA_ENVIRONMENT?.toLowerCase();
    if (!env) {
        throw new MpesaConfigurationError('Missing required environment variable: MPESA_ENVIRONMENT (must be "sandbox" or "production")');
    }
