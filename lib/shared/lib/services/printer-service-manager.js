 ...
 */
import { createAuthorityModeValidator, isPrinterDriverRequired, isESCPOSActive, isPrinterStatusMonitoringActive } from './printer-authority-validator';
/**
 * Default implementation of PrinterServiceManager
 */
export class DefaultPrinterServiceManager {
     ...
     */
    isServiceRequired(venueMode, authorityMode) {
    ...
    async detectDrivers() {
        ...
        const driversRequired = this.isServiceRequired(this.serviceConfig.venueMode, this.serviceConfig.authorityMode);
     ...
     */
    getInstallationGuidance(platform) {
    ...
    async establishConnection(config) {
        ...
        if (!isESCPOSActive(this.serviceConfig.authorityMode)) {
            throw new Error(`Printer connection not available for ${this.serviceConfig.authorityMode} authority mode. ` +
                'Printer services are only active for POS authority modes.');
        }
     ...
     */
    async testPrinter(connection) {
        // Verify printer testing is active for this authority mode
        if (!isESCPOSActive(this.serviceConfig.authorityMode)) {
            throw new Error(`Printer testing not available for ${this.serviceConfig.authorityMode} authority mode. ` +
    ...
    async *monitorStatus(connection) {
        ...
        if (!isPrinterStatusMonitoringActive(this.serviceConfig.authorityMode)) {
            throw new Error(`Printer status monitoring not available for ${this.serviceConfig.authorityMode} authority mode. ` +
 ...
 */
export function createPrinterServiceManager(config) {
 ...
 */
export function shouldInitializePrinterService(venueMode, authorityMode) {
    return isPrinterDriverRequired(venueMode, authorityMode);
}
