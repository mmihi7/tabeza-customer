...
describe('M-Pesa Configuration Loader', () => {
    ...
    describe('loadMpesaConfig', () => {
        ...
        it('should throw clear error for missing environment variable', () => {
            ...
            expect(() => loadMpesaConfig()).toThrow('Missing required environment variable: MPESA_ENVIRONMENT');
        });
        it('should throw clear error for multiple missing variables', () => {
            process.env.MPESA_ENVIRONMENT = 'sandbox';
            // Missing MPESA_CONSUMER_KEY and MPESA_CONSUMER_SECRET
            expect(() => loadMpesaConfig()).toThrow(MpesaConfigurationError);
            expect(() => loadMpesaConfig()).toThrow('Missing required M-Pesa environment variables: MPESA_CONSUMER_KEY, MPESA_CONSUMER_SECRET, MPESA_BUSINESS_SHORTCODE, MPESA_PASSKEY, MPESA_CALLBACK_URL');
        });
        it('should throw error for invalid environment value', () => {
            ...
            process.env.MPESA_CALLBACK_URL = 'https://example.com/callback';
            expect(() => loadMpesaConfig()).toThrow(MpesaConfigurationError);
            expect(() => loadMpesaConfig()).toThrow('Invalid MPESA_ENVIRONMENT: "invalid". Must be "sandbox" or "production"');
        });
        it('should validate business shortcode format', () => {
            ...
            process.env.MPESA_CALLBACK_URL = 'https://example.com/callback';
            expect(() => loadMpesaConfig()).toThrow(MpesaConfigurationError);
            expect(() => loadMpesaConfig()).toThrow('MPESA_BUSINESS_SHORTCODE must be 5-7 digits');
        });
        it('should validate consumer key length', () => {
            ...
            process.env.MPESA_CALLBACK_URL = 'https://example.com/callback';
            expect(() => loadMpesaConfig()).toThrow(MpesaConfigurationError);
            expect(() => loadMpesaConfig()).toThrow('MPESA_CONSUMER_KEY must be at least 10 characters');
        });
        it('should validate callback URL format', () => {
            ...
            expect(() => loadMpesaConfig()).toThrow(MpesaConfigurationError);
            expect(() => loadMpesaConfig()).toThrow('MPESA_CALLBACK_URL must be a valid URL');
        });
        it('should require HTTPS callback URL in production', () => {
            ...
            expect(() => loadMpesaConfig()).toThrow(MpesaConfigurationError);
            expect(() => loadMpesaConfig()).toThrow('MPESA_CALLBACK_URL must use HTTPS in production environment');
        });
