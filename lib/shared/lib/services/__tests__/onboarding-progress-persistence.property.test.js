...
import { OnboardingStateManager, OnboardingStep, createOnboardingProgress, saveOnboardingProgress, restoreOnboardingProgress, clearOnboardingProgress } from '../onboarding-state-management';
...
describe('Property Tests: Onboarding Progress Persistence', () => {
    ...
    test('Property 13: Onboarding Progress Persistence - Save/Restore Consistency', async () => {
        ...
        fc.record({
            ...
            venueMode: fc.option(fc.constantFrom('basic', 'venue'), { nil: null }),
            authorityMode: fc.option(fc.constantFrom('pos', 'tabeza'), { nil: null }),
    ...
    test('Property: Progress Expiry Handling', async () => {
        await fc.assert(fc.asyncProperty(fc.string({ minLength: 1, maxLength: 50 }), async (barId) => {
    ...
    test('Property: Step Transition Validation', () => {
        fc.assert(fc.property(fc.string({ minLength: 1, maxLength: 50 }), fc.constantFrom(...Object.values(OnboardingStep)), fc.constantFrom(...Object.values(OnboardingStep)), (barId, fromStep, toStep) => {
    ...
    test('Property: Core Truth - Basic Mode Requires POS Authority', () => {
        fc.assert(fc.property(fc.string({ minLength: 1, maxLength: 50 }), fc.constantFrom('pos', 'tabeza'), (barId, authorityMode) => {
            const progress = createOnboardingProgress(barId);
            ...
            if (authorityMode === 'pos') {
                ...
                const hasAuthorityError = validation.errors.some(e => e.code === 'BASIC_MODE_REQUIRES_POS' || e.field === 'authority_mode');
    ...
    test('Property: Core Truth - Venue Mode Authority Flexibility', () => {
        fc.assert(fc.property(fc.string({ minLength: 1, maxLength: 50 }), fc.constantFrom('pos', 'tabeza'), (barId, authorityMode) => {
            ...
            const hasInvalidAuthorityError = validation.errors.some(e => e.code === 'VENUE_MODE_INVALID_AUTHORITY');
    ...
    test('Property: Progress Completion Consistency', () => {
        fc.assert(fc.property(fc.string({ minLength: 1, maxLength: 50 }), fc.constantFrom(...Object.values(OnboardingStep)), (barId, step) => {
            const progress = createOnboardingProgress(barId);
    ...
    test('Property: Timestamp Consistency', async () => {
        await fc.assert(fc.asyncProperty(fc.string({ minLength: 1, maxLength: 50 }), fc.constantFrom(...Object.values(OnboardingStep)), async (barId, newStep) => {
            const progress = createOnboardingProgress(barId);
