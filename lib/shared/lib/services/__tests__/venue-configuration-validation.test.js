 ...
 */
import { validateVenueConfiguration, generateCorrectedConfiguration, validateConfigurationChange, isValidCoreConfiguration, getConfigurationDescription, getThemeConfiguration, getDefaultMigrationConfiguration } from '../venue-configuration-validation';
describe('Venue Configuration Validation Service', () => {
    ...
    describe('validateConfigurationChange', () => {
        test('should validate valid configuration change', () => {
            ...
            expect(result.isValid).toBe(true);
            expect(result.errors).toHaveLength(0);
            expect(result.warnings).toContain('Switching from Tabeza to POS authority will require POS integration setup. ' +
        ...
        test('should warn about destructive Venue to Basic change', () => {
            ...
            const result = validateConfigurationChange(currentConfig, newConfig);
            expect(result.isValid).toBe(true);
            expect(result.warnings).toContain('Switching from Venue to Basic mode will disable customer ordering and menus. ' +
