 ...
 */
import { createPrinterServiceManager, shouldInitializePrinterService } from '../printer-service-manager';
describe('Printer Service Manager', () => {
    ...
    describe('getInstallationGuidance', () => {
        it('should provide Windows installation guidance', () => {
            ...
            const manager = createPrinterServiceManager(config);
            const platform = { os: 'windows', browser: 'chrome', version: '1.0', supportsDrivers: true };
        ...
        it('should provide macOS installation guidance', () => {
            ...
            const manager = createPrinterServiceManager(config);
            const platform = { os: 'macos', browser: 'safari', version: '1.0', supportsDrivers: true };
            const guidance = manager.getInstallationGuidance(platform);
            expect(guidance.downloadUrl).toContain('tabeza.co.ke');
        ...
        it('should provide Linux installation guidance', () => {
            ...
            const manager = createPrinterServiceManager(config);
            const platform = { os: 'linux', browser: 'firefox', version: '1.0', supportsDrivers: true };
            const guidance = manager.getInstallationGuidance(platform);
            expect(guidance.downloadUrl).toContain('tabeza.co.ke');
        ...
        it('should provide mobile platform guidance (iOS)', () => {
            ...
            const manager = createPrinterServiceManager(config);
            const platform = { os: 'ios', browser: 'safari', version: '1.0', supportsDrivers: false };
            const guidance = manager.getInstallationGuidance(platform);
            expect(guidance.instructions.some(instruction => instruction.includes('does not support printer driver installation'))).toBe(true);
            expect(guidance.instructions.some(instruction => instruction.includes('desktop computer'))).toBe(true);
        });
        it('should provide mobile platform guidance (Android)', () => {
            ...
            const manager = createPrinterServiceManager(config);
            const platform = { os: 'android', browser: 'chrome', version: '1.0', supportsDrivers: false };
            const guidance = manager.getInstallationGuidance(platform);
            expect(guidance.instructions.some(instruction => instruction.includes('does not support printer driver installation'))).toBe(true);
        });
    });
