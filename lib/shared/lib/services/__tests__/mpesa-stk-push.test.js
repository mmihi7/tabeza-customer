 ...
 */
import { sendSTKPush, STKPushError, getSTKPushStats, resetSTKPushStats, sendSTKPushWithStats } from '../mpesa-stk-push';
import { getOAuthToken } from '../mpesa-oauth';
import { loadMpesaConfig } from '../mpesa-config';
// Mock dependencies
jest.mock('../mpesa-oauth');
jest.mock('../mpesa-config');
const mockGetOAuthToken = getOAuthToken;
const mockLoadMpesaConfig = loadMpesaConfig;
// Mock fetch globally
...
describe('STK Push Service', () => {
    ...
    describe('sendSTKPush', () => {
        ...
        it('should successfully send STK Push request', async () => {
            ...
            expect(mockFetch).toHaveBeenCalledWith(mockConfig.stkPushUrl, expect.objectContaining({
                method: 'POST',
                headers: {
        ...
        it('should handle Safaricom API errors', async () => {
            ...
            await expect(sendSTKPush(validRequest)).rejects.toThrow('STK Push rejected by Safaricom: Insufficient funds');
        ...
        it('should handle HTTP errors', async () => {
            ...
            await expect(sendSTKPush(validRequest)).rejects.toThrow('STK Push request failed: 400 Bad Request');
        });
        it('should handle network errors', async () => {
            mockFetch.mockRejectedValue(new TypeError('Failed to fetch'));
            await expect(sendSTKPush(validRequest)).rejects.toThrow('Network error: STK Push failed after 3 attempts');
        ...
        it('should fail after maximum retries', async () => {
            ...
            await expect(sendSTKPush(validRequest)).rejects.toThrow(/STK Push failed after 3 attempts/);
            expect(mockFetch).toHaveBeenCalledTimes(3);
        });
        it('should handle invalid response format', async () => {
            ...
            await expect(sendSTKPush(validRequest)).rejects.toThrow('Invalid STK Push response format from Safaricom API');
