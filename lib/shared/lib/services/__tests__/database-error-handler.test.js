 ...
 */
import { withDatabaseErrorHandling, withOnboardingErrorHandling, withVenueConfigErrorHandling, withMigrationErrorHandling, createUserErrorMessage, isTemporaryError, isPermanentError } from '../database-error-handler';
...
describe('Database Error Handler Service', () => {
    ...
    describe('withOnboardingErrorHandling', () => {
        it('should use onboarding-specific configuration', async () => {
            ...
            const result = await withOnboardingErrorHandling(mockOperation, 'complete_setup', { barId: '123' });
            expect(result.success).toBe(true);
    ...
    describe('withVenueConfigErrorHandling', () => {
        it('should use venue config-specific configuration', async () => {
            ...
            const result = await withVenueConfigErrorHandling(mockOperation, 'update_mode', { venueId: '123' });
            expect(result.success).toBe(false);
    ...
    describe('withMigrationErrorHandling', () => {
        it('should use migration-specific configuration with conservative retries', async () => {
            ...
            const result = await withMigrationErrorHandling(mockOperation, 'migrate_venue', { venueId: '123' });
            expect(result.success).toBe(false);
