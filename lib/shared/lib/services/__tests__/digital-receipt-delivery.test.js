 ...
 */
import { createDigitalReceiptDeliveryService } from '../digital-receipt-delivery';
...
describe('DigitalReceiptDeliveryService', () => {
    ...
    describe('convertPrintDataToDigitalReceipt', () => {
        it('should convert print data to digital receipt format', () => {
            const result = service.convertPrintDataToDigitalReceipt(samplePrintData, 'bar-123', 'tab-456');
        ...
        it('should handle items without quantity', () => {
            ...
            const result = service.convertPrintDataToDigitalReceipt(printData, 'bar-123', 'tab-456');
    ...
    describe('deliverToCustomer', () => {
        it('should successfully deliver digital receipt to customer', async () => {
            const result = await service.deliverToCustomer(samplePrintData, 'tab-456', 5, 'Customer A', 'bar-123');
        ...
        it('should handle delivery failure', async () => {
            ...
            mocksupabaseClient.from.mockImplementationOnce(errorMock.from);
            const result = await service.deliverToCustomer(samplePrintData, 'tab-456', 5, 'Customer A', 'bar-123');
            expect(result.success).toBe(false);
            expect(result.error).toContain('Database error');
        });
        it('should log delivery attempt', async () => {
            await service.deliverToCustomer(samplePrintData, 'tab-456', 5, 'Customer A', 'bar-123');
    ...
    describe('deliverToMultipleCustomers', () => {
        it('should deliver to multiple customers', async () => {
            ...
            const results = await service.deliverToMultipleCustomers(samplePrintData, customers, 'bar-123');
        ...
        it('should handle partial failures', async () => {
            ...
            mocksupabaseClient.from.mockImplementation(mixedMock.from);
            const customers = [
                { tabId: 'tab-1', tabNumber: 1, customerIdentifier: 'Customer 1' },
                { tabId: 'tab-2', tabNumber: 2, customerIdentifier: 'Customer 2' },
                { tabId: 'tab-3', tabNumber: 3, customerIdentifier: 'Customer 3' }
    ...
    describe('retryFailedDelivery', () => {
        it('should retry failed delivery', async () => {
            ...
            const serviceWithRetry = createDigitalReceiptDeliveryService(supabaseUrl, supabaseKey, customRetryConfig);
            ...
            mocksupabaseClient.from.mockImplementation(retryMock.from);
            const result = await serviceWithRetry.retryFailedDelivery(samplePrintData, 'tab-456', 5, 'Customer A', 'bar-123', 0);
            expect(result.success).toBe(true);
        ...
        it('should stop after max retries', async () => {
            ...
            mocksupabaseClient.from.mockImplementation(failMock.from);
            const result = await serviceWithRetry.retryFailedDelivery(samplePrintData, 'tab-456', 5, 'Customer A', 'bar-123', 0);
            expect(result.success).toBe(false);
        });
    });
    describe('delivery history', () => {
        ...
        it('should filter history by bar ID', async () => {
            await service.deliverToCustomer(samplePrintData, 'tab-1', 1, 'Customer 1', 'bar-123');
            await service.deliverToCustomer(samplePrintData, 'tab-2', 2, 'Customer 2', 'bar-456');
    ...
    describe('delivery statistics', () => {
        it('should calculate delivery statistics', async () => {
            ...
            // Successful delivery
            await testService.deliverToCustomer(samplePrintData, 'tab-stats-1', 1, 'Customer 1', 'bar-stats');
            ...
            mocksupabaseClient.from.mockImplementationOnce(failMock.from);
            await testService.deliverToCustomer(samplePrintData, 'tab-stats-2', 2, 'Customer 2', 'bar-stats');
