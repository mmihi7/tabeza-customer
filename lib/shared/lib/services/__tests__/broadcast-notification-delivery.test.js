...
class MockBroadcastSystem {
    constructor() {
        ...
        this.notificationDeliveryLog = [];
    }
    addClient(clientId, barId, userId) {
    ...
    getConnectedClientsForBar(barId) {
        return Array.from(this.connectedClients.values())
            .filter(client => client.barId === barId);
    }
    async broadcastPaymentNotification(barId, notification) {
...
describe('Broadcast Notification Delivery Properties', () => {
    ...
    test('Property 7: All connected staff members receive payment notifications simultaneously', async () => {
        ...
        fc.record({
            barId: fc.uuid(),
            connectedClients: fc.array(fc.record({
                clientId: fc.uuid(),
            ...
            const deliveredTo = await mockBroadcastSystem.broadcastPaymentNotification(barId, paymentNotification);
    ...
    test('Property: Broadcast delivery respects multi-tenant isolation', async () => {
        await fc.assert(fc.asyncProperty(fc.record({
            ...
            const deliveredTo = await mockBroadcastSystem.broadcastPaymentNotification(targetBar.barId, paymentNotification);
    ...
    test('Property: No connected clients are missed in broadcast delivery', async () => {
        await fc.assert(fc.asyncProperty(fc.record({
            ...
            for (const notification of notifications) {
                const delivered = await mockBroadcastSystem.broadcastPaymentNotification(barId, notification);
    ...
    test('Property: Broadcast delivery handles dynamic client connections', async () => {
        await fc.assert(fc.asyncProperty(fc.record({
            barId: fc.uuid(),
            stableClients: fc.array(fc.record({
                clientId: fc.uuid(),
                userId: fc.option(fc.uuid(), { nil: undefined })
            }), { minLength: 2, maxLength: 10 }),
            dynamicClients: fc.array(fc.record({
                clientId: fc.uuid(),
                userId: fc.option(fc.uuid(), { nil: undefined }),
