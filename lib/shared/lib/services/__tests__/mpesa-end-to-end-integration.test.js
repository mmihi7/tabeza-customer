...
jest.mock('../phoneValidation', () => ({
    validateKenyanPhoneNumber: jest.fn((phone) => ({
...
describe('M-Pesa End-to-End Integration Test', () => {
    ...
    describe('Complete Payment Flow Integration', () => {
        it('should complete full payment flow from initiation to callback processing', async () => {
            ...
            expect(console.log).toHaveBeenCalledWith('Payment verification passed:', expect.objectContaining({
                statusVerified: true,
                amountVerified: true,
                expectedAmount: 500,
                actualAmount: 500
    ...
    describe('Callback Idempotency', () => {
        it('should handle duplicate callbacks gracefully through database constraints', async () => {
            ...
            expect(console.log).toHaveBeenCalledWith('Callback idempotency: Payment already processed:', expect.objectContaining({
                checkoutRequestId: testCheckoutRequestId,
        ...
        it('should process second callback with different result gracefully', async () => {
            ...
            expect(console.log).toHaveBeenCalledWith('Payment already processed, skipping callback:', expect.objectContaining({
                paymentId: testPaymentId,
    ...
    describe('Error Handling and Edge Cases', () => {
        ...
        it('should handle payment not found during callback processing', async () => {
            ...
            expect(console.log).toHaveBeenCalledWith('Payment not found for CheckoutRequestID (graceful handling):', expect.objectContaining({
                checkoutRequestId: 'nonexistent_checkout_id'
            }));
        });
    });
