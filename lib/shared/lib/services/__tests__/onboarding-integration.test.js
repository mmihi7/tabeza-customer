...
import { checkOnboardingStatus, completeOnboarding, updateVenueConfiguration, migrateExistingVenue, saveOnboardingProgress, restoreOnboardingProgress, clearOnboardingProgress, validateOnboardingPrerequisites } from '../onboarding-operations';
...
describe('Complete Onboarding Workflows Integration Tests', () => {
    ...
    describe('New Venue Onboarding Workflows', () => {
        describe('Basic Mode Onboarding Flow', () => {
            it('should complete full Basic mode onboarding workflow', async () => {
                ...
                expect(mockLocalStorage.setItem).toHaveBeenCalledWith(`tabeza_onboarding_progress_${testBarId}`, JSON.stringify(progress));
                ...
                const completionResult = await completeOnboarding(mockSupabaseClient, testBarId, basicConfig);
                ...
                expect(mockLocalStorage.removeItem).toHaveBeenCalledWith(`tabeza_onboarding_progress_${testBarId}`);
        ...
        describe('Venue Mode Onboarding Flows', () => {
            it('should complete Venue + POS authority onboarding workflow', async () => {
                ...
                const completionResult = await completeOnboarding(mockSupabaseClient, testBarId, venueConfig);
                expect(completionResult.success).toBe(true);
        ...
        describe('Progress Persistence Integration', () => {
            it('should restore and continue onboarding from saved progress', async () => {
                ...
                expect(mockLocalStorage.getItem).toHaveBeenCalledWith(`tabeza_onboarding_progress_${testBarId}`);
            ...
            it('should handle expired progress gracefully', async () => {
                ...
                expect(mockLocalStorage.setItem).toHaveBeenCalledWith(`tabeza_onboarding_progress_${testBarId}`, JSON.stringify(freshProgress));
            });
        });
    });
    ...
    describe('Configuration Change Workflows', () => {
        it('should complete configuration change from Venue+Tabeza to Venue+POS', async () => {
            ...
            const updateResult = await updateVenueConfiguration(mockSupabaseClient, testBarId, currentConfig, newConfig);
    ...
    describe('Error Handling and Recovery Scenarios', () => {
        ...
        it('should handle validation errors during configuration changes', async () => {
            ...
            const result = await updateVenueConfiguration(mockSupabaseClient, testBarId, currentConfig, invalidConfig);
            expect(result.success).toBe(false);
    ...
    describe('Audit Logging Integration', () => {
        ...
        it('should log configuration change events with before/after states', async () => {
            ...
            await updateVenueConfiguration(mockSupabaseClient, testBarId, currentConfig, newConfig);
        ...
        it('should log validation failures with detailed error information', async () => {
            ...
            expect(console.error).toHaveBeenCalledWith('❌ Onboarding completion failed:', expect.stringContaining('Invalid configuration'));
        });
    });
    describe('Core Truth Constraint Enforcement', () => {
        ...
        it('should enforce Venue mode allows both POS and Tabeza authority', async () => {
            // Test Venue + POS
            const venuePosConfig = {
                venue_mode: 'venue',
                authority_mode: 'pos'
            };
            const completedVenuePOS = {
                id: testBarId,
                name: 'Test Restaurant',
                onboarding_completed: true,
                venue_mode: 'venue',
            ...
            // Test Venue + Tabeza
            const venueTabezaConfig = {
                venue_mode: 'venue',
                authority_mode: 'tabeza'
            };
            const completedVenueTabeza = {
                id: testBarId,
                name: 'Test Restaurant',
                onboarding_completed: true,
                venue_mode: 'venue',
