 ...
 */
async function parseWithDeepSeek(receiptText, barId, documentName) {
    try {
 ...
 */
function mapDeepSeekToParsedReceipt(deepseekResponse, rawText) {
    ...
    const items = Array.isArray(deepseekResponse.items)
        ? deepseekResponse.items.map((item) => ({
 ...
 */
function parseWithRegex(receiptText) {
 ...
 */
export async function parseReceipt(receiptText, barId, documentName) {
