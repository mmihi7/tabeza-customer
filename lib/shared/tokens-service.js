...
export class TokensService {
    ...
    async getBalance(userId) {
        ...
        if (error) {
            ...
            console.error('Token balance error details:', JSON.stringify(error, null, 2));
            return null;
        }
     ...
     */
    async awardFirstConnectionTokens(userId, venueId // Required for security
    ) {
        try {
            ...
            // Award tokens
            const { error } = await this.supabaseClient.rpc('award_tokens', {
                p_user_id: userId,
     ...
     */
    async awardOrderTokens(userId, venueId, // Required for RLS security
    orderId, orderValue) {
        try {
            // Calculate tokens using database function
            const { data: tokensAmount, error: calcError } = await this.supabaseClient.rpc('calculate_order_tokens', {
                p_user_id: userId,
                p_venue_id: venueId, // Required for RLS security
            ...
            const { data: countData } = await this.supabase
                ...
                .single();
            // Award tokens
            const { error: awardError } = await this.supabaseClient.rpc('award_tokens', {
                p_user_id: userId,
            ...
            if (venueId) { // Only increment if we have a venue
                await this.supabaseClient.rpc('increment_monthly_order_count', {
                    p_user_id: userId,
                    p_venue_id: venueId, // Required for RLS security
                });
            }
    ...
    async redeemReward(userId, rewardId) {
        try {
            ...
            const { data: code, error: codeError } = await this.supabaseClient.rpc('generate_redemption_code');
            if (codeError || !code) {
            ...
            // Deduct tokens
            const { error: deductError } = await this.supabaseClient.rpc('award_tokens', {
                p_user_id: userId,
