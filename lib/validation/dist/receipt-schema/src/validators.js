...
exports.validateReceiptSession = validateReceiptSession;
exports.validateReceiptEvent = validateReceiptEvent;
exports.validateCompleteSession = validateCompleteSession;
 ...
 */
function validateReceiptSession(session) {
    const errors = [];
    const warnings = [];
    let score = 100;
 ...
 */
function validateReceiptEvent(event) {
    const errors = [];
    const warnings = [];
    let score = 100;
    // Required fields
    ...
    // Totals validation
    if (!(0, session_1.validateEventTotals)(event.totals)) {
        errors.push({
            field: 'totals',
 ...
 */
function validateCompleteSession(completeSession) {
    const errors = [];
    const warnings = [];
    let score = 100;
    // Validate session
    ...
    // Validate events
    completeSession.events.forEach((event, index) => {
    ...
    const sequences = completeSession.events.map((e) => e.sequence).sort((a, b) => a - b);
    ...
    if (completeSession.totals) {
        if (!(0, session_1.validateSessionTotals)(completeSession.totals, completeSession.events)) {
            errors.push({
                field: 'totals',
 ...
 */
function validateBusinessRules(completeSession) {
    const errors = [];
    const warnings = [];
    let score = 100;
    ...
    if (totalAmount > 10000) { // KES 10,000+
        const hasPaymentReference = completeSession.events.some(event => event.payment && event.payment.reference);
    ...
    if (completeSession.session.merchant.kra_pin) {
        const hasProperTax = completeSession.events.every((event) => event.totals.tax >= 0 && event.totals.tax <= event.totals.subtotal * 0.2 // Max 20% tax
        );
        if (!hasProperTax) {
            warnings.push({
                field: 'events',
