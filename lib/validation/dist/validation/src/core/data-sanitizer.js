...
exports.sanitizeReceiptData = sanitizeReceiptData;
exports.sanitizePhoneNumber = sanitizePhoneNumber;
exports.sanitizeAmount = sanitizeAmount;
exports.sanitizeText = sanitizeText;
exports.normalizeReceiptData = normalizeReceiptData;
exports.applySanitizationRules = applySanitizationRules;
const constants_1 = require("../constants");
/**
 * Sanitize receipt data by removing/fixing common issues
 */
function sanitizeReceiptData(receipt, options = {
    trimWhitespace: true,
    ...
    try {
        ...
        if (sanitized.events && Array.isArray(sanitized.events)) {
            sanitized.events.forEach((event, index) => {
...
function sanitizeText(text) {
    if (typeof text !== 'string')
        return text;
    return text
        .replace(constants_1.VALIDATION_PATTERNS.CONTROL_CHARS, '')
        .replace(constants_1.VALIDATION_PATTERNS.EXCESSIVE_WHITESPACE, ' ')
        .replace(constants_1.VALIDATION_PATTERNS.LEADING_TRAILING_WHITESPACE, '');
}
/**
 * Normalize receipt data structure
 */
function normalizeReceiptData(receipt) {
 ...
 */
function applySanitizationRules(data, rules = []) {
    const rulesToApply = rules.length > 0
        ? constants_1.SANITIZATION_RULES.filter(rule => rules.includes(rule.name))
        : constants_1.SANITIZATION_RULES;
    let sanitized = data;
...
function sanitizeEventData(event, eventIndex) {
    const changes = [];
    // Sanitize items
...
function sanitizeTotalsData(totals, prefix = 'totals') {
    const changes = [];
...
function sanitizeMetadata(receipt) {
    const changes = [];
...
function applySanitizationRuleRecursively(obj, rule) {
